...
class PDFService {
      ...
      const processNextImage = () => {
        ...
        try {
          ...
          if (settings.watermark.enabled && settings.watermark.text) {
            this.addWatermark(doc, width, height, settings.watermark);
          }
  ...
  addWatermark(doc, width, height, watermark) {
    try {
      doc.save();
      ...
      console.warn('Could not add watermark:', watermarkError.message);
    }
  }
  ...
  async saveToCloudStorage(ctx, filePath, fileName) {
    try {
      const { CLOUD_CHANNEL } = require('../config/constants');
